...
class student{
    ...
    static void leaderboard(){
        ...
        for(int i = 0; i<5; i++){
            cout<<"NAME: "<<user_objects[i].name<<endl<<"USERNAME: "<<user_objects[i].username<<endl<<"SCORE: "<<user_objects[i].score<<endl<<endl;
        }
    }
...
class admin{
    ...
    void removequestions(){
        string file1 = "questions.txt";
        string file2 = "ques_temp.txt";
        fstream fname;
        fname.open(file1, ios::in);
        if(fname.is_open()){
            fstream temp_file;
            temp_file.open(file2, ios::out);
            if(temp_file.is_open()){
                string temp_total;
                int total;
                stringstream ss;
                getline(fname,temp_total);
                ss << temp_total;
                ss >> total;
                total--;
                temp_file<<total<<endl;
                vector <question> allquestions = question::getquestions();
                for(int i = 0; i<allquestions.size(); i++){
                    cout<< i+1<< ". " << allquestions[i].display_questions() << endl;
                }
                int remove_question_number;
                cout<<"ENTER QUESTION NUMBER TO DELETE: ";
                cin>>remove_question_number;
                int count = 1;
                string file_question;
                while(getline(fname, file_question)){
                    if(count == remove_question_number){
                        count++;
                        string line;
                        getline(fname, line);
                        getline(fname, line);
